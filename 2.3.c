...
void *Control1(void *arg) {
	...
	double ek, vk, gain_p, gain_i, gain_d, motor_position;
	double prev_i = 0;
	double prev_d = 0; 
	double prev_er = 0;
	double prev_vk = 0;
	...
	for (k=0; k<no_of_samples; k++) {
		...
		gain_i = prev_i+(Kp/Ti)*prev_er*Ts;				//calculate integral gain
		gain_d = ((Ti/(N*Ts)+Td)*prev_d)+(((Kp*Td*N)/((N*Ts)+Td))*(ek-prev_er));		//calculate differential gain
		...
		prev_d = gain_d;
		prev_er = ek;
		//prev_uk = uk;							//for use with anti-windup
	}
